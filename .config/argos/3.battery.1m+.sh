...
echo_monospace(){
	for line in $@; do echo "$line | font=Monospace"; done
}
...
echo "⚡️ $POWER_SUPPLY_CAPACITY % $STATUS | color=$COLOR"
...
if [[ $ARGOS_MENU_OPEN="true" ]]; then
	acpi -V | grep "Battery"
	echo "---"
	echo_monospace $(sensors)
	echo "---"
	echo_monospace $(nvidia-smi -q -d temperature | grep "Current Temp" | head -n 1 | sed "s/\ *GPU/GPU/")
fi
